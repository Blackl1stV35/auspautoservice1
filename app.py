...
# ---------------------------------------------------------------------------
# Helper: reload all data into session_state
# ---------------------------------------------------------------------------
def refresh_data():
    """(Re)load every CSV into st.session_state via data_store getters."""
...
# ===================================================================
    ...
    with col1:
        req_file = st.file_uploader(
            "📄 ไฟล์เบิกวัสดุสิ้นเปลือง", type=["xlsx", "xls"],
            key="req_upload")
    with col2:
            ...
            with st.spinner("⏳ กำลังประมวลผล... อาจใช้เวลาสักครู่"):
                try:
                    ...
                    ds.log_audit("admin", "ETL",
                                 f"อัปโหลดไฟล์ req={bool(req_file)} pur={bool(pur_file)}")
                    ds.git_commit("ETL: อัปโหลดไฟล์ Excel ใหม่")

                    st.markdown(
                        '<div class="success-box">✅ ประมวลผลสำเร็จ!</div>',
                        unsafe_allow_html=True)
        ...
        if st.button("✅ ยืนยันเบิกวัสดุ", type="primary",
            ...
            try:
                ...
                st.markdown(
                    ...
                    f'✅ บันทึกสำเร็จ! {selected_emp} เบิก '
                    f'{selected_mat} x{quantity}</div>',
                    unsafe_allow_html=True)
                st.balloons()
            except Exception as e:
                st.error(f"❌ เกิดข้อผิดพลาด: {e}")
...
# ===================================================================
    ...
    if stock.empty:
        ...
        cols_show = [c for c in ["item_name", "current_qty", "last_updated"]
                     if c in display.columns]
        rename_map = {"item_name": "วัสดุ",
                      "current_qty": "คงเหลือ",
...
# ===================================================================
    ...
    if req.empty:
        ...
        view_mode = st.radio("มุมมอง", ["ภาพรวมทุกคน", "รายบุคคล"],
                             horizontal=True)
...
# ===================================================================
    ...
    st.info("ระบบจะแจ้งเตือนช่างที่เบิกวัสดุมากผิดปกติ "
            "(มากกว่า 2 เท่าของค่าเบี่ยงเบนมาตรฐาน)")
    ...
    req = st.session_state["requisitions"]
    if not req.empty and "employee_name" in req.columns:
